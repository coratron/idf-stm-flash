...
int initFATFS(void)
{
    flashLogInfo(FLASH_CONFIG, "Initializing FATFS");
    ...
    esp_err_t ret = esp_vfs_fat_spiflash_mount("/", "ffat", &conf, &s_wl_handle);
    ...
    if (ret != ESP_OK)
    {
        ...
        {
            flashLogError(FLASH_CONFIG, "Failed to find FATFS partition");
        }
        else
        {
            flashLogError(FLASH_CONFIG, "Failed to initialize FATFS (%s)", esp_err_to_name(ret));
        }
        return 0;
    }
